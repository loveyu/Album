...
class LyCore
{
	...
	public function load_lib()
	{
		...
		if (count($list) == 1) {
			if (!isset($_class[$name]) && isset($this->lib[$name]))
				return $this->lib[$name];
			if (!isset($_class[$name])) return false;
			return $_class[$name];
		} else return $_class;
	}
	...
	{
		if ($this->get_config('system', '404_page') && !$this->flag_404) {
			$this->flag_404 = true;
			call_user_func_array(array($this,"load"),$this->config['system']['404_page']);
		} else {
			$this->core_view('404');
		}
	}
	...
	public function view($__CORE_file, $__CORE_param = array(), $__CORE_once = false)
	{
		//加载默认变量
		//$_CORE = &$this;
		foreach (array_keys($this->lib) as $__CORE_name) {
			$__CORE_TMP = '_' . $__CORE_name;
			$$__CORE_TMP = &$this->lib[$__CORE_name];
		}
		$_CONFIG = &$this->config;
		unset($__CORE_name, $__CORE_TMP);
